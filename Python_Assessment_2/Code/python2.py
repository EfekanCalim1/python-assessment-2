...
def four(input1, input2):
	output = ''.join(''.join(f for f in tup) for tup in zip(input1, input2))
	return output
...
def five():
	import random
	number = [random.randint(50, 100)*2 for i in range(5)]
...
def nine(string1, string2):
	count = {string1[i]: 0 for i in range(len(string1).upper())}

	for i in range(len(string1).upper()):
		count[string1[i]] += 1

	for i in range(len(string2).upper()):
...
def ten(X,Y):
	X, Y = int(input("Enter row and column value: ").split(", "))
